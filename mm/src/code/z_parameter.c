...
void Interface_DrawItemButtons(PlayState* play) {
    ...
    static TexturePtr sCUpLabelTexturesPAL[] = {
        gTatlCUpPALENGTex,
        gTatlCUpPALGERTex,
        gTatlCUpPALFRATex,
        gTatlCUpPALESPTex,
    ...
    if (interfaceCtx->tatlCalling && !IS_PAUSED(&play->pauseCtx) && (play->csCtx.state == CS_STATE_IDLE) &&
        ...
        if (sCUpInvisible == 0) {
            ...
            if (ResourceMgr_GetGameRegion(0) == GAME_REGION_PAL) {
                gDPLoadTextureBlock_4b(OVERLAY_DISP++, sCUpLabelTexturesPAL[gSaveContext.options.language - 1],
                                       G_IM_FMT_IA, 32, 12, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP,
                                       G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
            } else {
                gDPLoadTextureBlock_4b(OVERLAY_DISP++, sCUpLabelTextures[gSaveContext.options.language], G_IM_FMT_IA,
                                       32, 12, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK,
                                       G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
            }
        ...
        if (interfaceCtx->perfectLettersState[i] != PERFECT_LETTERS_STATE_OFF) {
            ...
            if (ResourceMgr_GetGameRegion(0) == GAME_REGION_PAL) {
                OVERLAY_DISP = Gfx_DrawTexQuad4b(
                    OVERLAY_DISP, sPerfectLettersTexturesPAL[gSaveContext.options.language - 1][i], G_IM_FMT_I, 32, 33,
                    0);
            } else {
                OVERLAY_DISP =
                    Gfx_DrawTexQuad4b(OVERLAY_DISP, sPerfectLettersTextures[i], G_IM_FMT_I, 32, 33, 0);
            }
